    ...
    <div>
      <GridLayout tracks={participantTracks} style={{ height: "calc(100vh)" }}>
        <>
          <ParticipantTile className="border-2 border-slate-600">
            ...
            <ParticipantName
              style={{}}
              className="text-white bg-black bg-opacity-50 px-3  rounded-lg font-bold absolute top-5 left-1/4"
            />
          </ParticipantTile>
        </>
      </GridLayout>
      ...
      {agentTrack.length > 0 && (
        <div className="absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 z-50 rounded-full">
          <IfaOrb
            trackRef={tracks.find((track) => track.participant.isAgent)}
          />
        </div>
      )}
...
const IfaOrb = ({ trackRef }: { trackRef: any }) => {
  const { elementProps } = useParticipantTile({ trackRef, htmlProps: {} });

  return (
    <div
      {...elementProps}
      className={`participant-tile p-6 absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 z-50 rounded-full ${
        (elementProps as any)["data-lk-speaking"] ? "speaking" : ""
      }`}
    >
      <div
        style={{ borderRadius: 360 }}
        className="shadow-[0_0_15px_0px_-5px] shadow-violet-600 rounded-full  p-4 bg-gradient-radial from-white to-slate-400"
      >
        <svg
          width="42"
          height="42"
