...
import { useRoomContext, VideoTrack, useRemoteParticipants, useParticipants, useTracks, GridLayout, ParticipantTile, ParticipantName, ConnectionStateToast, RoomAudioRenderer } from "@livekit/components-react";
    ...
    <>
      ...
      {agentTrack.length > 0 && (
        <div className='absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 z-50 rounded-full'>
          <ParticipantTile style={{ borderRadius: 350 }} trackRef={tracks.find((track) => track.participant.isAgent)} className='flex mx-auto align-middle justify-center'>
            <div className='shadow-[0_0_15px_0px_-5px] shadow-violet-600 rounded-full m-4 p-4 bg-gradient-radial from-white to-slate-400'>
